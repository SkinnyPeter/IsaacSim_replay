...
def _setup_logging(log_cfg: LogConfig):
    ...
    if log_cfg.file_enabled:
        ...
        # Attach only to src.* so Isaac Sim's own logging is excluded from the file
        src_logger = logging.getLogger("src")
        src_logger.setLevel(logging.DEBUG)  # must be DEBUG so the handler's level is the actual gate
...
def main():
    scene_path, data_path, robot_cfg, sim_cfg, vis_cfg, seg_cfg, log_cfg, rec_cfg = load_config(BASE_DIR / "config" / "config.yaml", base_dir=BASE_DIR)
    ...
    simulator = Simulator(
        ...
        sim_config=sim_cfg, vis_config=vis_cfg, robot_config=robot_cfg, seg_config=seg_cfg, recording_config=rec_cfg
    )
    simulator.play()
